...
{
	   for(var x = 0; x < document.frmusergroupedit.elements.length; x++ ) 
	   {	 
		var y = document.frmusergroupedit.elements[x]; 
	     if( y.name != 'CHECKALL') {
			y.checked = document.frmusergroupedit.CHECKALL.checked; 
		}
	   } 
}
...
<form name="frmusergroupedit" action="adm_mst_usergroupeditcfm.php" method="post">

	<table width="100%" >
		<tr>
			<td class="headingI">Edit Group</td>
		</tr>
	</table>
		...
		<? while ($row = mysql_fetch_array($rs_usergroup)) { ?>
			<table width="80%" align="center">
				<tr>
					...
					<td width="60%" >
						<input type="text" name="txtusergroup_code"  readonly class="textbox_code" value="<?=$row['usergroup_code']?>">
				...
				<tr>
					...
					<td width="40%" class="labels">
						<input type="text" name="txtusergroup_name" class="textbox_name" value="<?=$row['usergroup_name']?>">
						<label class="mandatory">*</label>
					</td>
				</tr>
		...
		<table width="80%" align="center">
				<tr>
					<td colspan="3" align="center">	
						<input type="hidden" name="txtusergroup_gid" value="<?=$_GET['txtusergroup_gid']?>">
